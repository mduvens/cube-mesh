...
let sphereM = []
for(var i = 0; i<5; i++){
sphereM[i] = new THREE.MeshNormalMaterial({wireframe: true})
}
var sphere = []
//********************/
for(var i = 0; i<5; i++){
sphere[i] = new THREE.Mesh(sphereGeo, sphereM[i])
}
...
function init(){
    ...
    // **** DOM EVENTS ( THREEx + TWEENMAX) ****** / .bind ... function() == .addEventListener... event =>
    let offset1 = 10
     //for(i=0;i<5; i++){
    domEvents.addEventListener(sphere[1], 'mouseover', event =>{
        sphere[1].material.wireframe = false
    })
     domEvents.bind(sphere[1], 'mouseout', function(){
        //sphere[1].material.wireframe = true
        sphere[1].material.wireframe = true
     })
     domEvents.bind(sphere[3], 'mouseover', function(){
        sphere[3].material.wireframe = false
     })
     domEvents.bind(sphere[3], 'mouseout', function(){
        sphere[3].material.wireframe = true
     })/*
    ...
    domEvents.bind(sphere[4], 'click', function(){
        tl.to(sphere[4].scale , 1 , { z: 4, x: 4, y: 4}).to(sphere[4].position , 1 , { x: 60},"-=1").to(sphere[4].position , 1 , { x: 30}).to(sphere[4].scale, 1, {z:1,x:1,y:1},"-=1")
    })
